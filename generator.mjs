...
const createModuleSpec = (name, typeOfId, column, columType) => {
  return [
    `import { Test, TestingModule } from '@nestjs/testing';`,
    ...
    `import { ${capitalize(name)}Module } from './${name}.module';`,
    ...
    `  let savedId: ${typeOfId === 'increment' ? 'number' : 'string'};`,
    ``,
    `  beforeAll(async ()=>{`,
    `    const db = newDb({`,
    ...
    `  };`,
    `  it('create', async () => {`,
    `    const keyName = 'create${capitalize(name)}';`,
    ` `,
    `    const gqlQuery = {`,
    '      query: `',
    '        mutation ($input: ${Create' +
      capitalize(name) +
      'Input.prototype.constructor.name}!) {',
    '          ${keyName}(input: $input) {',
    `            id`,
    ...
    `        savedId = ${typeOfId === 'increment' ? 'Number(id)' : 'id'};`,
    `        expect(data[keyName]).toMatchObject(created);`,
    `      });`,
    `  });`,
    ` `,
    `  it('getMany', async () => {`,
    `    const keyName = 'getMany${capitalize(name)}List';`,
    ` `,
    `    const gqlQuery = {`,
    '      query: `',
    `        query {`,
    ...
    ` `,
    `  it('getOne', async () => {`,
    `    const keyName = 'getOne${capitalize(name)}';`,
    ` `,
    `    const gqlQuery = {`,
    '      query: `',
    ...
    ` `,
    `  it('update', async () => {`,
    `    const keyName = 'update${capitalize(name)}';`,
    ` `,
    `    const gqlQuery = {`,
    '      query: `',
    `        mutation ($id: ${
      typeOfId === 'increment' ? 'Float' : 'String'
    }!,` +
      '$input: ${Update' +
      capitalize(name) +
      'Input.prototype.constructor.name}!) {',
    ...
    ` `,
    `  it('delete', async () => {`,
    `    const keyName = 'delete${capitalize(name)}';`,
    ` `,
    `    const gqlQuery = {`,
    '      query: `',
    `        mutation ($id: ${
...
const addSource = async (name, test, typeOfId, column, type) => {
  const dir = `./src/${name}`;
  fs.mkdirSync(dir);
  ...
  if (test !== 'no') {
    ...
    fs.writeFileSync(
      `${dir}/${name}.module.spec.ts`,
      createModuleSpec(name, typeOfId, column, type),
    );
  }
};
...
const start = async () => {
  ...
  addSource(table, test, typeOfId, coulumn, type);
